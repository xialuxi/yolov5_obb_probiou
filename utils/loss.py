...
from utils.probiou_loss import ProbiouLoss
...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        BCEtheta = nn.CrossEntropyLoss(reduction="none")
        ...
        angle_proj = np.linspace(0, self.angle_max, self.angle_max + 1)
        self.angle_proj = torch.from_numpy(angle_proj).to(device) * self.half_pi_bin
        self.angle_proj = self.angle_proj.float()
        self.angle_proj = torch.tensor(self.angle_proj, requires_grad=False)
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                ptheta = ps[:, class_index:]
                pangle = F.softmax(ptheta, dim=1).matmul(self.angle_proj).reshape(-1, 1)
                ...
                with torch.no_grad():
                    tangle = (ttheta[i] / self.half_pi_bin).clip(0, self.angle_max - 0.01)
                    target_left = tangle.long()
                    target_right = target_left + 1
                    weight_left = target_right.float() - tangle
                    weight_right = 1 - weight_left
                    
                    target_left = target_left.squeeze()
                    target_right = target_right.squeeze()
                
                loss_left = self.BCEtheta(ptheta, target_left) * weight_left
                loss_right = self.BCEtheta(ptheta, target_right) * weight_right
                ltheta += (loss_left + loss_right).mean()
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            ttheta.append(theta_labels)

        # return tcls, tbox, indices, anch
        return tcls, tbox, indices, anch, ttheta #, ttheta
